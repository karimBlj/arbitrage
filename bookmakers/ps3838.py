...
def get_page(competition : Competition):
	...
	driver = webdriver.Chrome("/Users/karim/Documents/trading_MFT/chromedriver", chrome_options=options)
	driver.set_window_size(1920, 1080)
	driver.get(url['url'])
	driver.find_element(By.CSS_SELECTOR, "[data-league=\"{}\"]".format(url['code'])).click()
	found = 0
	while (found >= 0):
		try:
			driver.find_element(By.CSS_SELECTOR, ".OneXTwo_0")
			found = -1
		except:
			if (found >= 1000):
				driver.quit()
...
def get_games(competition : Competition) -> T_get_games:
	games = []
